...
- 模型：`paraphrase-multilingual-mpnet-base-v2`（sentence-transformers，768维，多语言支持，较 MiniLM 更强）
- 算法：KNN（3近邻，余弦距离），按 label 去重后取最高权重
- 训练数据：`synthetic_training_data.py`（55条样本，9类）
- 模型文件：`/home/ai/openclaw-classifier/python-service/models/scene_classifier.pkl`
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `label` | string\|null | 场景标签，`null` 表示置信度过低 |
| `confidence` | float | 相似度 (0-1)，1 表示完全匹配 |
| `meta` | string | `confident` / `low_confidence` / `ambiguous` |
| `intercepted` | boolean | 是否被硬拦截（触发高风险规则） |
| `response` | string | 附加的 LLM 文本描述（可选） |
...
```json
{
  "label": "high_risk_automated",
  "confidence": 0.94,
  "meta": "confident",
  "intercepted": true,
  "response": "[!WARNING] 此操作具有风险。\n[LLM响应] 基于你的输入，建议操作如下：..."
}
```
